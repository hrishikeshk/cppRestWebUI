...
void Buy::handleRequest(const Request& request, Response& response) {
    ...
    std::tuple<OperationStatus, std::tuple<int, std::string, std::string > > op_result = ctrl.buy(userVec[0], 
																									passVec[0],
																									stockVec[0],
																									qty,
																									price);
	...
	resp_json[L"Buy_Request_Status"] = web::json::value::string(utility::conversions::to_string_t(std::get<1>(resultTuple)));
	resp_json[L"Buy_Request_Comment"] = web::json::value::string(utility::conversions::to_string_t(std::get<2>(resultTuple)));
